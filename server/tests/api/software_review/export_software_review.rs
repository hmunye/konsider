...
#[tokio::test]
async fn export_software_review_successful() -> Result<()> {
    ...
    let export_software_review_response = server
        .get_request(
            &format!("{}/{}/export", software_review_url, software_review_id),
            Some(&token.unwrap()),
        )
        .await?;
    assert_eq!(200, export_software_review_response.status().as_u16());

    Ok(())
}

#[tokio::test]
async fn export_software_request_using_missing_token_rejected() -> Result<()> {
    let server = spawn_server().await?;
    let export_software_review_url =
        format!("{}/api/v1/reviews/{}/export", server.addr, Uuid::new_v4());

    let export_software_review_response = server
        .get_request(&export_software_review_url, None)
        .await?;
    assert_eq!(401, export_software_review_response.status().as_u16());

    Ok(())
}

#[tokio::test]
async fn export_software_with_invalid_id_rejected() -> Result<()> {
    let server = spawn_server().await?;
    let login_url = format!("{}/api/v1/auth/login", server.addr);
    let export_software_review_url =
        format!("{}/api/v1/reviews/{}/export", server.addr, Uuid::new_v4());
    ...
    let export_software_review_response = server
        .get_request(&export_software_review_url, Some(&token.unwrap()))
        .await?;
    // Returns a 404 status code to indicate the user does not exist
    assert_eq!(404, export_software_review_response.status().as_u16());
